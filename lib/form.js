...
function form() {
  let routines = Array.prototype.slice.call(arguments);

  if (routines.length && Array.isArray(routines[0])) {
    routines = routines[0];
  }

  let options = form._options;
    ...
    async.each(routines, function(routine, cb) {
      routine.run(mergedSource, req.form, options, function(err, result) {
        ...
        result.forEach(function (error) {
          ...
          if (error.index) {
            map[name][error.index] = error.error;
          } else {
            map[name].push(error.error);
          }
        });
